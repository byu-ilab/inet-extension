...
Define_Module(httptHTMLServer);

void httptHTMLServer::initialize()
{
...
void httptHTMLServer::finish()
{
	httptHTMLServerBase::finish();
...
void httptHTMLServer::handleMessage(cMessage *msg)
{
	...
	httptHTMLServerBase::handleMessage(msg);
}

void httptHTMLServer::socketEstablished(int connId, void *yourPtr)
{
    EV_INFO << "connected socket with id=" << connId << endl;
	socketsOpened++;
}

void httptHTMLServer::socketDataArrived(int connId, void *yourPtr, cPacket *msg, bool urgent)
{
	if ( yourPtr==NULL )
	{
...
void httptHTMLServer::socketPeerClosed(int connId, void *yourPtr)
{
	if ( yourPtr==NULL )
	{
		EV_ERROR << "Socket establish failure. Null pointer" << endl;
...
void httptHTMLServer::socketClosed(int connId, void *yourPtr)
{
...
void httptHTMLServer::socketFailure(int connId, void *yourPtr, int code)
{
...
void httptHTMLServer::updateDisplay() {
	httptHTMLServerBase::updateDisplay();
	if ( ev.isGUI() )
	{
		char buf[1024];
