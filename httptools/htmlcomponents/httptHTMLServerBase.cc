...
#include "httptHTMLServerBase.h"


void httptHTMLServerBase::initialize()
{
	...
	EV_DEBUG << "Initializing HTML server component\n";
	/* handled in httptServerBase::initialize()
	...
	*/
	...
	// handled by httptServerBase::initialize()
...
void httptHTMLServerBase::finish()
{
...
void httptHTMLServerBase::updateDisplay()
{
	if ( ev.isGUI() )
	{
		char buf[1024];
...
void httptHTMLServerBase::handleMessage(cMessage *msg)
{
	// Override in derived classes
	updateDisplay();
}

/*// handled by httptServerBase
cMessage* httptHTMLServerBase::handleReceivedMessage( cMessage *msg )
{
...
httptReplyMessage* httptHTMLServerBase::handleGetRequest( httptRequestMessage *request, string resource )
{
...
httptReplyMessage* httptHTMLServerBase::generateDocument( httptRequestMessage *request, const char* resource, int size )
{
...
httptReplyMessage* httptHTMLServerBase::generateResourceMessage( httptRequestMessage *request, string resource, CONTENT_TYPE_ENUM category )
{
	...
	*/
	if (scriptedMode)
	{
		...
		if (category==rt_text)
			replymsg->setByteLength(rdTextResourceSize->get());// Set the resource size
		else if ( category==rt_image )
			replymsg->setByteLength(rdImageResourceSize->get());// Set the resource size
	}
	//replymsg->setKind(HTTPT_RESPONSE_MESSAGE);
	char szReply[512];
...
/*//handled by httptServerBase
httptReplyMessage* httptHTMLServerBase::generateErrorReply( httptRequestMessage *request, int code )
{
	char szErrStr[32];
	sprintf(szErrStr,"HTTP/1.1 %.3d %s", code, httpPhraseFromCode(code).c_str());
...
string httptHTMLServerBase::generateBody()
{
...
/*//handled by httptServerBase
void httptHTMLServerBase::registerWithController()
{
...
*/
void httptHTMLServerBase::readSiteDefinition(string file)
{
	...
	while(!std::getline(tracefilestream, line).eof())
	{
		...
		else
		{
			...
			if ( siteSection )
			{
				if ( res.size()<2 || res.size()>3 ) //TODO make it >= 3
...
string httptHTMLServerBase::readHtmlBodyFile( string file, string path )
{
